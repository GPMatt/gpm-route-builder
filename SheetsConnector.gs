...
function rowToWO_(row) {
  ...
  return {
    ...
    statusNotes:      toPlain_(row[WO_COL.STATUS_NOTES - 1]),
    maintenanceLimit: toPlain_(row[WO_COL.MAINTENANCE_LIMIT - 1]),
    assignedUser:     toPlain_(row[WO_COL.ASSIGNED_USER - 1]),
    status:           toPlain_(row[WO_COL.STATUS - 1]),
    createdAt:        toPlain_(row[WO_COL.CREATED_AT - 1]),
    appFolioApptText: apptIsLive ? toPlain_(row[WO_COL.APPFOLIO_SCHED_TEXT - 1]) : '',
